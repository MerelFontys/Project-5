    ...
    def check_column(df, columname, valid_values, nan_allowed=False):
        for index, row in df.iterrows():  # index allows you to find out in which "numbered" row a value is located
            ...
            if cel_value not in valid_values:
                st.write(f"Row {index} (Excel-row {index + 2}), column '{columname}': '{cel_value}' is not a valid value")  # index + 2 omdat het een excelbestand is en dan zie je in welke rij in het excel bestand de foutieve waarde staat
