...
# Every check below APPENDS its findings to a shared "issues" list, instead of
# writing to the page directly. This lets us show exactly ONE summary message:
# green if the list stays empty, red listing every problem if it doesn't.
...
def check_column(df, columname, valid_values, issues, nan_allowed=False):
    for index, row in df.iterrows():  # index allows you to find out in which "numbered" row a value is located
        ...
        if pd.isna(cel_value):  # is the cell empty? let's check if it's allowed to be empty
            if not nan_allowed:
                issues.append(f"Row {index} (Excel-row {index + 2}), column '{columname}': empty value is not allowed")
            continue

        if cel_value not in valid_values:
            issues.append(f"Row {index} (Excel-row {index + 2}), column '{columname}': '{cel_value}' is not a valid value")


def check_line_by_idle_material(df, issues):  # are the cells that are supposed to be empty, empty?
    ...
    for index, row in df.iterrows():
        if row['activity'] in activities_without_line:
            if pd.notna(row['line']):  # if the 'line' column of this row actually contains a value (i.e. not an empty/NaN cell)
                issues.append(f"Row {index} (Excel-row {index + 2}): activity is '{row['activity']}' but 'line' is not empty (value: {row['line']})")


def check_charging_negative(df, issues):  # when the bus is charging, is the energy consumption negative?
    for index, row in df.iterrows():
        if row['activity'] == 'charging':
            if row['energy consumption'] >= 0:
                issues.append(f"Row {index} (Excel-row {index + 2}): activity is 'charging' but energy consumption is {row['energy consumption']} (supposed to be negative)")


def check_service_trip_has_line(df, issues):  # does every service trip have a line?
    for index, row in df.iterrows():
        if row['activity'] == 'service trip' and pd.isna(row['line']):
            issues.append(f"Row {index} (Excel-row {index + 2}): activity is 'service trip' but 'line' is empty")


def check_positive_energy(df, activities, issues):  # every activity except for charging needs to have a positive energy consumption
    for index, row in df.iterrows():
        if row['activity'] in activities and row['energy consumption'] < 0:
            issues.append(f"Row {index} (Excel-row {index + 2}): activity is '{row['activity']}' "
                           f"but energy consumption is {row['energy consumption']} (should be positive)")


def check_idle_consumption(df, issues, expected_value=5.0):
    for index, row in df.iterrows():
        if row['activity'] == 'idle' and row['energy consumption'] != expected_value:
            issues.append(f"Row {index} (Excel-row {index + 2}): activity is 'idle' but energy consumption "
                           f"is {row['energy consumption']} (expected {expected_value})")


def check_duplicate_rows(df, issues):  # are there any exact the same rows? (double rows)
    duplicates = df[df.duplicated(keep=False)]

    for index, row in duplicates.iterrows():
        issues.append(f"Row {index} (Excel-row {index + 2}) is a duplicate row: {row.to_dict()}")


def remove_start_equals_end(df):  # if start time equals end time, delete the whole row - this is expected cleaning, not a data error
...
def check_location_continuity(df, issues):  # is the end location the same location where the bus starts the next trip
    for bus_id, group in df.groupby('bus', sort=False):
        ...
        for i in range(len(group) - 1):
            ...
            if current_end != next_start:
                issues.append(f"Bus {bus_id}: row ends at '{current_end}' ({group.loc[i, 'end time']}) "
...
if uploaded_file is not None:  # you cannot run this unless a file has been uploaded
    ...
    if not missing_columns:
        ...
        # collect every issue found by every check in this one list
        issues = []

        check_column(df_busplanning_copy, 'start location', valid_locations, issues)
        check_column(df_busplanning_copy, 'end location', valid_locations, issues)
        check_column(df_busplanning_copy, 'activity', valid_activies, issues)
        check_column(df_busplanning_copy, 'line', valid_line_values, issues, nan_allowed=True)

        check_line_by_idle_material(df_busplanning_copy, issues)
        check_charging_negative(df_busplanning_copy, issues)
        ...
        check_service_trip_has_line(df_busplanning_copy, issues)
        check_positive_energy(df_busplanning_copy, ['material trip', 'service trip'], issues)
        check_idle_consumption(df_busplanning_copy, issues)
        check_duplicate_rows(df_busplanning_copy, issues)

        df_busplanning_copy, removed_count = remove_start_equals_end(df_busplanning_copy)

        check_location_continuity(df_busplanning_copy, issues)

        # --- Exactly ONE summary message about actual data problems: red if any were found, green otherwise ---
        if issues:
            issue_list_markdown = "\n".join(f"- {issue}" for issue in issues)
            st.error(
                f"**{len(issues)} data quality issue(s) found:**\n\n"
                f"{issue_list_markdown}\n\n"
                f"Please correct the invalid values in the source file to values that are accepted, then upload it again."
            )
        else:
            st.success("All data was loaded successfully and can be used to generate a bus plan.")

        # Removing rows with no useful data (start time = end time) is expected, routine cleaning,
        # not a data error - so it gets a plain informational line, not a colored blob.
        if removed_count > 0:
            st.write(f"{removed_count} row(s) with no useful data (start time equals end time) were removed.")
        ...
        display_df = df_busplanning_copy.copy()
